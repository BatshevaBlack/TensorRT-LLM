...
class DisaggregatedTester:
    ...
    def start_context_server(self, gpu_id: int, port: int) -> int:
        ...
        cmd = [
            ...
            "trtllm-serve", self.config['model_path'],
            "--host", "localhost", "--port", str(port), "--backend", "pytorch",
            "--extra_llm_api_options", self.config['extra_llm_api_path'],
    ...
    def start_disaggregated_service(self) -> int:
        ...
        cmd = [
            ...
            "-c", self.config['disagg_config_path'],
            "-m", self.config['etcd_config_path']
        ]
    ...
    def wait_for_server_health(self, port: int, timeout: int = 120) -> bool:
        ...
        while time.time() - start_time < timeout:
            ...
            if process_key and self.processes[process_key].poll() is not None:
                logger.error(f"Server process on port {port} exited prematurely with code {self.processes[process_key].returncode}")
    ...
    def wait_for_disagg_service_health(self, port: int = 8000, timeout: int = 120) -> bool:
        """Wait for disaggregated service to be healthy."""
        url = f"http://localhost:{port}/health"
        start_time = time.time()
        logger.info(f"Waiting for disaggregated service on port {port} to be healthy...")
        
        while time.time() - start_time < timeout:
            try:
                response = requests.get(url, timeout=5)
                if response.status_code == 200:
                    logger.info(f"Disaggregated service on port {port} is healthy")
                    return True
            except requests.RequestException:
                pass
            ...
            if self.processes["disagg_service"].poll() is not None:
                logger.error(f"Disaggregated service exited prematurely with code {self.processes['disagg_service'].returncode}")
        ...
        logger.error(f"Timed out waiting for disaggregated service on port {port}")
        return False
    ...
    def run_client_test(self) -> bool:
        ...
        cmd = [
            "python3", 
            f"{self.config['client_script_path']}",
            "-c", self.config['disagg_config_path'],
            "-p", self.config['prompts_path']
        ]
    ...
    def kill_server(self, port: int) -> bool:
        ...
        process_key = next((k for k in self.processes if str(port) in k), None)
        if not process_key:
            logger.warning(f"No process found for port {port} in tracked processes")
        ...
        if process.poll() is None:  # Still running
            ...
            logger.warning(f"Process already exited with code {process.returncode}")
            return False
    ...
    def run_complete_test(self) -> bool:
        ...
        try:
            ...
            logger.info("Waiting for the service to recognize the server removal...")
            time.sleep(20)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Automated Disaggregated Testing")
    parser.add_argument("--model-path", default="TinyLlama/TinyLlama-1.1B-Chat-v1.0", 
                        help="Path to the model")
    parser.add_argument("--extra-llm-api-path", 
                        default="TensorRT-LLM/examples/disaggregated/extra-llm-api-config.yml",
                        help="Path to extra LLM API config")
    parser.add_argument("--etcd-config-path", 
                        default="TensorRT-LLM/examples/disaggregated/etcd_config.yaml",
                        help="Path to etcd config")
    parser.add_argument("--disagg-config-path", 
                        default="disagg_config.yaml",
                        help="Path to disaggregation config")
    parser.add_argument("--client-script-path", 
                        default="TensorRT-LLM/examples/disaggregated/clients/disagg_client.py",
                        help="Path to client script")
    parser.add_argument("--prompts-path", 
                        default="TensorRT-LLM/examples/disaggregated/clients/prompts.json",
